...
describe('unit 4 answer key', () => {
  ...
  it('188 — A(BCD) = 56/4 = 14, BE/BD = 3/7, A(BEF) = 3/7 · 1/2 · 14 = 3, 14 − 3 = 11 → C', () =>
    expect(answerOf('quadrilaterals-188')).toBe('C'));
  it('189 — DG = 8, AG² = 36 + 64 = 100, FG = 10/2 = 5 = FB + DC → D', () =>
    expect(answerOf('quadrilaterals-189')).toBe('D'));
  it('190 — AM = 12, DN = 3, AD = 15, h² = 225 − 81 = 144, (15 + 6)·12/2 = 126 → D', () =>
    expect(answerOf('quadrilaterals-190')).toBe('D'));
  it('191 — EB = 6, 10h − 3h = 28, h = 4, (10 + 16)·4/2 = 52 → E', () =>
    expect(answerOf('quadrilaterals-191')).toBe('E'));
  it('192 — AB = 400/10 = 40, CE = CA/5, A(ADE) = A(BCE) = 400/5 = 80 → B', () =>
    expect(answerOf('quadrilaterals-192')).toBe('B'));
  it('193 — EG = 3·5 = 15, DG = 20, AG = 25, AH = 225/25 = 9 → A', () =>
    expect(answerOf('quadrilaterals-193')).toBe('A'));
  it('194 — AH = DC = 5, HB = 6√2/√2 = 6, x = 5 + 6 = 11 → B', () =>
    expect(answerOf('quadrilaterals-194')).toBe('B'));
  it('195 — DC = 2·3 = 6, EL = 3 + 5 = 8, AB = 2·8 = 16 → D', () =>
    expect(answerOf('quadrilaterals-195')).toBe('D'));
  it('196 — h = 6√2/√2 = 6, x·√3/2 = 6, x = 12/√3 = 4√3 → C', () =>
    expect(answerOf('quadrilaterals-196')).toBe('C'));
  it('197 — AH = (20 − 8)/2 = 6, h² = 100 − 36 = 64, h = 8, (8 + 20)·8/2 = 112 → E', () =>
    expect(answerOf('quadrilaterals-197')).toBe('E'));
  it('198 — ADC ∼ BAD, x² = 4·9 = 36, x = 6 → D', () =>
    expect(answerOf('quadrilaterals-198')).toBe('D'));
  it('199 — A/2 + D/2 = 90, AED = 90, AD² = 64 + 36 = 100, AD = 10 → A', () =>
    expect(answerOf('quadrilaterals-199')).toBe('A'));
  it('200 — KB = AH = 3, AB = 3 + 6 + 3 = 12, (6 + 12)·8/2 = 72 → B', () =>
    expect(answerOf('quadrilaterals-200')).toBe('B'));
  it('201 — AH = 4, HB = 16, h² = 4·16 = 64, h = 8, (4 + 20)·8/2 = 96 → D', () =>
    expect(answerOf('quadrilaterals-201')).toBe('D'));
  it('202 — FG = (6 + 18)/2 = 12, EH = 9, KL = 15, 9 + 12 + 15 = 36 → E', () =>
    expect(answerOf('quadrilaterals-202')).toBe('E'));
  it('203 — AB² = 3·12 = 36, AB = 6, x² = 36 + (12 − 3)² = 117 → D', () =>
    expect(answerOf('quadrilaterals-203')).toBe('D'));
  it('204 — A(ABCD) = A(PBCQ) = BC·EH = 9·8 = 72 → B', () =>
  ...
  it('212 — DA = DC, AC² = 90, HC = 3√10/2, DH = HC·3/9 = √10/2 → B', () =>
    expect(answerOf('quadrilaterals-212')).toBe('B'));
  it('213 — BF = 2·2 = 4, EF = 2·4 = 8, DF = 12, AF = √(25 + 144) = 13, x = 13 − 4 = 9 → B', () =>
    expect(answerOf('quadrilaterals-213')).toBe('B'));
  it('214 — AF = 12 + 3 = 15, BD = CF = √(225 − 81) = 12, 9·12/2 = 54 → D', () =>
    expect(answerOf('quadrilaterals-214')).toBe('D'));
  it('215 — BE = 8, AD = AB = 10, BED ≅ BCD, BC = 8, DC = 4, (10 + 4)·8/2 = 56 → C', () =>
    expect(answerOf('quadrilaterals-215')).toBe('C'));
  it('216 — AE = AD = 13, AB = 26, AH = √(169 − 144) = 5, x = 26 − 5 = 21 → D', () =>
    expect(answerOf('quadrilaterals-216')).toBe('D'));
  it('217 — CE = 2√3·√3 = 6, HE = 3, AH = 36/3 = 12, x = 12 + 3 = 15 → D', () =>
    expect(answerOf('quadrilaterals-217')).toBe('D'));
  it('218 — DC = 4k, AB = 9k, AD² = 36k², 39k² = 156, k = 2, x = 12 → D', () =>
    expect(answerOf('quadrilaterals-218')).toBe('D'));
  it('219 — DHC ~ BCA ratio 1/2, BC = 10, DC = 13, HC = 12, AC = 24, 120 + 60 = 180 → B', () =>
    expect(answerOf('quadrilaterals-219')).toBe('B'));
  it('220 — DC = DA = 8, AM = 6, 6/x = 8/12, x = 9 → E', () =>
    expect(answerOf('quadrilaterals-220')).toBe('E'));
  it('221 — h = (7 + 23)/2 = 15, AH = 8, AD = 17, 7 + 23 + 34 = 64 → C', () =>
    expect(answerOf('quadrilaterals-221')).toBe('C'));
  it('222 — 8/a = a/18, a = 12, AD = 20, (12 + 18)·20/2 = 300 → B', () =>
    expect(answerOf('quadrilaterals-222')).toBe('B'));
  it('223 — KE = 8, KC = 12, KB = 15, KA = 4 = 2x, x = 2 → C', () =>
    expect(answerOf('quadrilaterals-223')).toBe('C'));
  it('224 — (3a + b)/(a + 3b) = 3/5, 12a = 4b, a/b = 1/3 → B', () =>
    expect(answerOf('quadrilaterals-224')).toBe('B'));
  it('225 — DE = DA = x, EC = 2x, (20 − 2x)/20 = 2x/20, x = 5 → C', () =>
    expect(answerOf('quadrilaterals-225')).toBe('C'));
  it('226 — CB = CD = 13, BE = BC = 13, AB = 18, h² = 169 − 25, h = 12, 31·12/2 = 186 → D', () =>
    expect(answerOf('quadrilaterals-226')).toBe('D'));
  it('227 — ½·12²·sin 30° = 36 → A', () => expect(answerOf('quadrilaterals-227')).toBe('A'));
  it('228 — (3a + 4)(a + 4) = 128, a = 4, 8 × 16 twice → 16 × 16, 4·16 = 64 → B', () =>
    expect(answerOf('quadrilaterals-228')).toBe('B'));
  it('229 — AB = 25, KB = 15, 9-12-15 right, h = 108/15 = 7.2, 10·7.2/2 = 36 → E', () =>
    expect(answerOf('quadrilaterals-229')).toBe('E'));
