...
describe('unit 5 answer key', () => {
  ...
  it('175 — 48 = 4·AD, AD = 12, CD = 8 = r, 60° dilim: 32π/3 − 16√3 → D', () =>
    expect(answerOf('circles-175')).toBe('D'));
  it('176 — R − r = 2, R² − r² = 20, R + r = 10, r = 4 → B', () =>
    expect(answerOf('circles-176')).toBe('B'));
  it('177 — 36α = 16(α + 50), 20α = 800, α = 40 → C', () =>
    expect(answerOf('circles-177')).toBe('C'));
  it('178 — A(ABC) = S₁ + W = S₂ + W = π·36/2 = 18π → D', () =>
    expect(answerOf('circles-178')).toBe('D'));
  it('179 — AB = 8, 32 − 4π·90/360 − 16π/4 = 32 − 5π → A', () =>
  ...
  it('187 — 120/x = 5/2, 5x = 240, x = 48 → C', () => expect(answerOf('circles-187')).toBe('C'));
  it('188 — r² = a²/4 + a², a² = 4r²/5, (4/5)/(π/2) = 8/(5π) → B', () =>
    expect(answerOf('circles-188')).toBe('B'));
  it('189 — 16 = 2·MH, MH = 8, r = MK = 10, 25π − 10·4 = 25π − 40 → D', () =>
    expect(answerOf('circles-189')).toBe('D'));
  it('190 — m(COB) = m(AOD) = 130, 36π · 260/360 = 26π → D', () =>
    expect(answerOf('circles-190')).toBe('D'));
  it('191 — R = 4, r = 3, 2 · (8π − 9π/2) = 7π → A', () =>
  ...
  it('205 — R − r = 2, R² − r² = 20, R + r = 10, R = 6, çap 12 → C', () =>
    expect(answerOf('circles-205')).toBe('C'));
  it('206 — OD = 8, m(DOB) = 30, 64π/12 + ½ · 4 · 4√3 = 16π/3 + 8√3 → D', () =>
    expect(answerOf('circles-206')).toBe('D'));
  it('207 — ½ · AB · 12 = 36π/2, 6 · AB = 18π, AB = 3π → B', () =>
    expect(answerOf('circles-207')).toBe('B'));
  it('208 — DC = 2x, 6 + 2x = 10 + x, x = 4, 6 + 10 + 8 + 4 = 28 → E', () =>
    expect(answerOf('circles-208')).toBe('E'));
  it('209 — r = 6, 9π/2 + (72 − 18π)/2 = 36 − 9π/2 → E', () =>
    expect(answerOf('circles-209')).toBe('E'));
  it('210 — 256 + (x + 2)² = (10 + x)², x = 10, 192 − 50π → A', () =>
    expect(answerOf('circles-210')).toBe('A'));
  it('211 — S₁ = (16 − 4)π/4 = 3π, S₂ = π/4, 3π / (π/4) = 12 → C', () =>
    expect(answerOf('circles-211')).toBe('C'));
  it('212 — ABC eşkenar, |AB| = 8, 32π/3 − 8√3 − 4π = 20π/3 − 8√3 → B', () =>
    expect(answerOf('circles-212')).toBe('B'));
  it('213 — a² + b² = 36, ab/2 ≤ 9, 9π − 9 → C', () => expect(answerOf('circles-213')).toBe('C'));
  it('214 — HB = 3, HA = 6, R² − r² = 36 − 9 = 27 → B', () =>
    expect(answerOf('circles-214')).toBe('B'));
  it('215 — m(AOD) = 30, 64π/12 − ½ · 4 · 4 = 16π/3 − 8 → B', () =>
    expect(answerOf('circles-215')).toBe('B'));
  it('216 — AC = 15, r = 6, 54 − 3 · 36/4 = 27 → D', () =>
    expect(answerOf('circles-216')).toBe('D'));
  it('217 — 36 + 36 = 72 = |AB|², m(AOB) = 90, 27π + ½ · 6 · 6 = 27π + 18 → D', () =>
    expect(answerOf('circles-217')).toBe('D'));
  it('218 — AO = 8, 32π − 2 · π/2 + 18π = 49π → B', () =>
    expect(answerOf('circles-218')).toBe('B'));
  it('219 — OE = 2r, OC = r = 4, OB = 12, 144π · 30/360 = 12π → D', () =>
    expect(answerOf('circles-219')).toBe('D'));
  it('220 — (6 + 36π) − 6 = 36π, 36π / 6π = 6 → C', () =>
    expect(answerOf('circles-220')).toBe('C'));
  it('221 — OC = r, OA = r√3, r²√3/2 = 8√3, r² = 16, 16π · 3/4 = 12π → C', () =>
  ...
  it('229 — |BM| = 3, kesme 3π/2 − 9√3/4, (9√3/2 − 3π/2) + 2 · kesme = 3π/2 → B', () =>
    expect(answerOf('circles-229')).toBe('B'));
  it('230 — 4 − h = 4 − √(16 − h²), h = 2√2, 16π/4 − 4 · 2√2 = 4π − 8√2 → A', () =>
    expect(answerOf('circles-230')).toBe('A'));
  it('231 — 6 = 4p, p = 3/2, OAPB = 2 · ½ · 6 · 3/2 = 9, 9π − 9 → B', () =>
    expect(answerOf('circles-231')).toBe('B'));
  it('232 — |EF|² = 4 · 16, |EF| = 8, 50π / (20 · 8) = 5π/16 → A', () =>
    expect(answerOf('circles-232')).toBe('A'));
  it('233 — R = 6, dilim(BO₁D) = 9π/2, 18π − 9π/2 = 27π/2 → B', () =>
    expect(answerOf('circles-233')).toBe('B'));
  it('234 — 6 · 2πr = 2πR · 270/360, 12r = 3R/2, r/R = 1/8 → C', () =>
    expect(answerOf('circles-234')).toBe('C'));
