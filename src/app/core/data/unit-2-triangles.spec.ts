...
describe('unit 2 answer key', () => {
  ...
  it('435 — |DC| = 13, |DE|² = 169 − 25 = 144, |BD|² = 81 + 144 = 225, x = (2/3)·15 = 10 → D', () =>
    expect(answerOf('triangles-435')).toBe('D'));
  it('436 — |BG| = 6, 36 = 2m² ⇒ m² = 18, |BD|² = 3m² = 54, |BC| = 2·3√6 = 6√6 → E', () =>
    expect(answerOf('triangles-436')).toBe('E'));
  it('437 — |MC| = 8, |GM|² = 100 − 64 = 36, |BM| = 3·6 = 18, A = 16·18/2 = 144 → A', () =>
    expect(answerOf('triangles-437')).toBe('A'));
  it('438 — |BG| = 9, |GD| = 27, |KD| = 9, |LD| = 3, |GL| = 27 − 3 = 24 → B', () =>
    expect(answerOf('triangles-438')).toBe('B'));
  it('439 — A(AFG) = S, A(GDE) = 2S − 6S/4 = S/2, (3S/2)/6S = 1/4 → C', () =>
    expect(answerOf('triangles-439')).toBe('C'));
  ...
  it('447 — h = 3·2 = 6, |BC| = 7 + 11 = 18, A = 18·6/2 = 54 → C', () =>
    expect(answerOf('triangles-447')).toBe('C'));
  it('448 — |CD| = |CA| = 6, |BC| = 12, m(C) = 60°, |CE| = 4√3, |CK| = 3√3, x = √3 → B', () =>
    expect(answerOf('triangles-448')).toBe('B'));
  it('449 — |GD| = 5, |AD| = 15, |BD|² = 169 − 25 = 144, |BC| = 24, A = 24·15/2 = 180 → C', () =>
    expect(answerOf('triangles-449')).toBe('C'));
  it('450 — |FG| = |BE|/6, A(BDE) = S/4, S/24 = 3 ⇒ S = 72 → D', () =>
    expect(answerOf('triangles-450')).toBe('D'));
  it('451 — |CD| = 21, |BE| = 12, (5/4)x² = 441 + 144 = 585, x² = 468, x = 6√13 → C', () =>
    expect(answerOf('triangles-451')).toBe('C'));
  ...
  it('471 — |AC|/|CB| = 6/8, |EH| = 4, |FH| = 4·3/4 = 3, x = 8 − 3 = 5 → C', () =>
    expect(answerOf('triangles-471')).toBe('C'));
  it('472 — |CL|/|CD| = 3/9, |DL|/|DC| = 2/3, |BC| = 12·3/2 = 18 → D', () =>
    expect(answerOf('triangles-472')).toBe('D'));
  it('473 — ABD ∼ ACB, 8² = 4·|AC|, |AC| = 16, x = 16 − 4 = 12 → D', () =>
    expect(answerOf('triangles-473')).toBe('D'));
  it('474 — |AM| = 12, |ME| = 6, m(DME) = 60, DME eşkenar, x = 2·6 = 12 → E', () =>
    expect(answerOf('triangles-474')).toBe('E'));
  it('475 — |DH| = 2, |BH| = 2√3, |AB| = 6√3, |BC| = 3, |AC|² = 108 + 9 = 117 → C', () =>
    expect(answerOf('triangles-475')).toBe('C'));
  it('476 — |FE|/|BC| = 3/8, |ED|/|BC| = 1/4, oran = 3/2 → B', () =>
    expect(answerOf('triangles-476')).toBe('B'));

  it('477 — CED ∼ CAB, 5/8 = x/10, x = 25/4 → D', () =>
    expect(answerOf('triangles-477')).toBe('D'));
  it('478 — |AB| = |AC|, |DK|/|HC| = 4/6, 4/x = 2/3, x = 6 → B', () =>
    expect(answerOf('triangles-478')).toBe('B'));
  it('479 — Alan oranı 2/3, x² = 16·3/2 = 24, x = 2√6 → C', () =>
    expect(answerOf('triangles-479')).toBe('C'));
  it('480 — |BM| = 6, |MC| = 8, |BC| = 10, |GD| = 8·4/10 = 16/5 → C', () =>
    expect(answerOf('triangles-480')).toBe('C'));
  it('481 — 3/9 = 9/|AC|, |AC| = 27, x = 27 − 9 = 18 → D', () =>
    expect(answerOf('triangles-481')).toBe('D'));
  it('482 — 8/12 = 12/18, DCB ∼ BDA, x = 6·3/2 = 9 → B', () =>
    expect(answerOf('triangles-482')).toBe('B'));

  it('483 — ABE ∼ DCE, 6/15 = 4/x, x = 10 → D', () =>
    expect(answerOf('triangles-483')).toBe('D'));
  it('484 — oran 4/6, |AC| = 9, |EC| = 3, |BC| = 12, toplam 15 → E', () =>
    expect(answerOf('triangles-484')).toBe('E'));
  it('485 — ABE ∼ CDE, 4/x = 8/6, x = 3 → B', () =>
    expect(answerOf('triangles-485')).toBe('B'));
  it('486 — m(DEF) = 180 − 50 − 70 = 60 = m(B), x = 180 − 60 − 50 = 70 → D', () =>
    expect(answerOf('triangles-486')).toBe('D'));
  it('487 — |DB| = |DE| = 4, |AB| = 10, 6/10 = 4/x, x = 20/3 → A', () =>
    expect(answerOf('triangles-487')).toBe('A'));
  it('488 — AED ∼ ACB, 6/(5 + x) = 5/10, x = 7 → C', () =>
    expect(answerOf('triangles-488')).toBe('C'));

  it('489 — |AB|/|AD| = 14/7 = |AC|/|AE| = 16/8 = 2, ABC ∼ ADE, x = 2 · 6 = 12 → B', () =>
    expect(answerOf('triangles-489')).toBe('B'));
  it('490 — |AB| = 12, BHE ∼ BAC, |BE| = 8 · 15/12 = 10, x = 12 − 10 = 2 → C', () =>
    expect(answerOf('triangles-490')).toBe('C'));
  it('491 — |DE|/|EF| = 3/4, 7k = 28, x = 4k = 16 → E', () =>
    expect(answerOf('triangles-491')).toBe('E'));
  it('492 — ADE ∼ ABC, |AD| = 12 · 4/10 = 24/5, x = 12 − 24/5 = 36/5 → A', () =>
    expect(answerOf('triangles-492')).toBe('A'));
  it('493 — AED ∼ ABC (E ↔ B), |AD|/|AC| = 1/2, |AC| = 16 → D', () =>
    expect(answerOf('triangles-493')).toBe('D'));
  it('494 — a/2 = 8/a, a = 4, |AB| = 4√3, |DE| = 2√3, toplam 6√3 → C', () =>
    expect(answerOf('triangles-494')).toBe('C'));

  it('495 — BDE ∼ BCA, 9/|BC| = 6/16, |BC| = 24, x = 24 − 6 = 18 → D', () =>
    expect(answerOf('triangles-495')).toBe('D'));
  it('496 — |FP|/|BC| = 15/20, |AC| = 16, |AP| = 12, |DE| = 12 · 10/15 = 8 → B', () =>
    expect(answerOf('triangles-496')).toBe('B'));
  it('497 — |FC| = 9/3 = 3, |AK|/|AC| = 3/5, |BC| = 15, |BF| = 15 − 3 = 12 → E', () =>
    expect(answerOf('triangles-497')).toBe('E'));
  it('498 — |AC| = 8, CED ∼ CAB, |CD| = 10 · 5/8 = 25/4, x = 8 − 25/4 = 7/4 → C', () =>
    expect(answerOf('triangles-498')).toBe('C'));
  it('499 — |CL|/|CA| = 1/4, |CD| = 20/4 = 5, |BD| = 20 − 5 = 15 → A', () =>
    expect(answerOf('triangles-499')).toBe('A'));
  it('500 — |CE|/|CA| = 9/15 = 3/5, |AE|/|AC| = 2/5, |DE| = 25 · 2/5 = 10 → E', () =>
    expect(answerOf('triangles-500')).toBe('E'));

  it('501 — |CE|/|CA| = 12/18 = 2/3, |AE|/|AC| = 1/3, |BC| = 3 · 8 = 24 → D', () =>
    expect(answerOf('triangles-501')).toBe('D'));
  it('502 — BDE ∼ BCA, 10/|BC| = 9/18, |BC| = 20, x = 20 − 9 = 11 → C', () =>
    expect(answerOf('triangles-502')).toBe('C'));
  it('503 — |AC| = 5k, |CL|/|CA| = 1/5, |CD| = 21/5, |BD| = 21 − 21/5 = 84/5 → B', () =>
    expect(answerOf('triangles-503')).toBe('B'));
  it('504 — |FP|/|BC| = 9/12, |AC| = 20, |AP| = 15, |DE| = 15 · 6/9 = 10 → E', () =>
    expect(answerOf('triangles-504')).toBe('E'));
  it('505 — |FC| = 12/4 = 3, |AK|/|AC| = 4/7, |BC| = 21, |BF| = 21 − 3 = 18 → B', () =>
    expect(answerOf('triangles-505')).toBe('B'));
  ...
  it('513 — DEC ∼ BAC, oran 4/12 = 1/3, A(DEC)/A(ABC) = 1/9, A(DEC)/A(ABED) = 1/8 → B', () =>
    expect(answerOf('triangles-513')).toBe('B'));
  it('514 — |DE|/|AB| = 4/10 = 2/5, x/(x + 9) = 2/5, x = 6 → C', () =>
    expect(answerOf('triangles-514')).toBe('C'));
  it('515 — |KE|/|CD| = 3/5 = |KE|/|BC|, y/(y + 8) = 3/5, y = 12 → D', () =>
    expect(answerOf('triangles-515')).toBe('D'));
  it('516 — |DC| = 10, DEC ∼ CBA, 6/9 = 10/|AC|, |AC| = 15, |AE| = 15 − 8 = 7 → C', () =>
    expect(answerOf('triangles-516')).toBe('C'));
  it('517 — |BE|/|BD| = 6/9 = 2/3, |BC| = 4 · 3/2 = 6, |FC| = 6 − 4 = 2 → C', () =>
    expect(answerOf('triangles-517')).toBe('C'));
  ...
  it('525 — [DE] // [AB], |CD|/|CB| = 4/10, A(ABD)/A(ABC) = 3/5 → C', () =>
    expect(answerOf('triangles-525')).toBe('C'));
  it('526 — |DC| = 10, 6/9 = 10/|BC|, |BC| = 15, x = 15 − 8 = 7 → B', () =>
    expect(answerOf('triangles-526')).toBe('B'));
  it('527 — |KF| = 6/2 = 3, |BD| = 3 · 3 = 9 → D', () =>
    expect(answerOf('triangles-527')).toBe('D'));
  it('528 — S/2 − S/6 − S/8 = 5S/24 = 10, S = 48 → C', () =>
    expect(answerOf('triangles-528')).toBe('C'));
  it('529 — |CA| = 16/2 = 8, |AD| = 2 · 3 = 6, |DC| = 2 → B', () =>
    expect(answerOf('triangles-529')).toBe('B'));
  it('530 — |AB| = |EC| = 10, |AB|/|DC| = 4/6, |DC| = 15, x = 5 → D', () =>
    expect(answerOf('triangles-530')).toBe('D'));

  it('531 — |AE| = a, |EK| = |KB| = 2a, 5a = 20, x = 4 · 4 = 16 → D', () =>
    expect(answerOf('triangles-531')).toBe('D'));
  it('532 — |AH| = 10, |DH| = 6, |EH| = √(100 − 36) = 8, x = 16 → E', () =>
    expect(answerOf('triangles-532')).toBe('E'));
  it('533 — AED ∼ CBA oran 1/2, |AB| = 6, |AE| = 4, 24 − 6 = 18 → B', () =>
    expect(answerOf('triangles-533')).toBe('B'));
  it('534 — |FE| = |BE|/4, |BF| = 3x = 18, x = 6 → C', () =>
    expect(answerOf('triangles-534')).toBe('C'));
  it('535 — |AD|² = 6 · 3 = 18, |DE| = 6, |BC| = 15, x² = 3 · 15 = 45 → D', () =>
    expect(answerOf('triangles-535')).toBe('D'));
  it('536 — |CB| = 3 · 15/5 = 9, |AB| = √(225 − 81) = 12, x = 12/3 = 4 → A', () =>
    expect(answerOf('triangles-536')).toBe('A'));

  it('537 — ADE ∼ BCA, 6/8 = x/12, x = 9 → C', () =>
    expect(answerOf('triangles-537')).toBe('C'));
  it('538 — |AE|/|AF| = |AF|/|AB|, |AB| = 144/9 = 16, x = 4 → D', () =>
    expect(answerOf('triangles-538')).toBe('D'));
  it('539 — |BC| = 3 · 5 = 15, |AC| = √(225 − 81) = 12, x = 12/4 = 3 → E', () =>
    expect(answerOf('triangles-539')).toBe('E'));
  it('540 — 4 · (|BD|/|DC|) · 1/2 = 1, |DB| = |BC|, (6/x) · 2/3 = 1, x = 4 → B', () =>
    expect(answerOf('triangles-540')).toBe('B'));
  it('541 — oranlar 1 : 2 : 5, (25 − 4)/1 = 21 → D', () =>
    expect(answerOf('triangles-541')).toBe('D'));
  it('542 — |BK| = |AB|, |AE| = |AK|/2, |DF| = 3|AK|/4, oran 2/3 → C', () =>
    expect(answerOf('triangles-542')).toBe('C'));

  it('543 — (6/4)(6/12)(|CA|/|AD|) = 1, |CA|/|AD| = 4/3, |AE|/|EB| = 3/7, oran 10/3 → D', () =>
    expect(answerOf('triangles-543')).toBe('D'));
  it('544 — 3/6 = |AD|/8, |AD| = 4, x² = 25 + 100 = 125, x = 5√5 → C', () =>
    expect(answerOf('triangles-544')).toBe('C'));
  it('545 — |EC| = 4, 3/2a = 4/(a + 5), a = 3, |BC| = 10, x = 6 → B', () =>
    expect(answerOf('triangles-545')).toBe('B'));
  it('546 — |NC| = 4, |EN|² = 12, |DN|² = 48 − 12 = 36, |AC| = 12, x = 10 → D', () =>
    expect(answerOf('triangles-546')).toBe('D'));
  it('547 — A(ABC) = 64s, S₂ = 16s, S₁ = 36s/4 = 9s, oran 9/16 → A', () =>
    expect(answerOf('triangles-547')).toBe('A'));
  it('548 — h = 6, 2x/12 = (6 − x)/6, x = 3 → E', () =>
    expect(answerOf('triangles-548')).toBe('E'));

  it('549 — |AC| = 6, |DH| = 4, |HC| = 3, |HE| = 1, x² = 16 + 1 = 17 → C', () =>
    expect(answerOf('triangles-549')).toBe('C'));
  it('550 — ABE ≅ BCD, |BA| = 4k, |BF| = 12k/5 = 12, k = 5, x = 25 − 12 = 13 → D', () =>
    expect(answerOf('triangles-550')).toBe('D'));
  it('551 — |EF|/|AB| = 6/18, |DF|/|AB| = 2/3, x = 12 · 2/5 = 4,8 → C', () =>
    expect(answerOf('triangles-551')).toBe('C'));
  it('552 — 8/6 = 16/12 = 4/3, DCE ∼ ABC, |DE| = 9 · 4/3 = 12 → B', () =>
    expect(answerOf('triangles-552')).toBe('B'));
  it('553 — |EB| = a/6, |DB| = 2c/3, taralı = ac/18 + 5ac/18 = ac/3 = 20, A = 60/2 = 30 → D', () =>
    expect(answerOf('triangles-553')).toBe('D'));
  ...
  it('561 — ABD: 35, 40, 105 ⇒ |BD| < |AD|, “|BD| > |AD|” yanlış → C', () =>
    expect(answerOf('triangles-561')).toBe('C'));
  it('562 — 8² + 15² = 289, 17 < |BC| < 23, 18..22 = 5 değer → B', () =>
    expect(answerOf('triangles-562')).toBe('B'));
  it('563 — 4 < x < 16 ve 3 < x < 11, 5 + 6 + 7 + 8 + 9 + 10 = 45 → D', () =>
    expect(answerOf('triangles-563')).toBe('D'));
  it('564 — m(ADB) > 90, |AB|² > 25 + 144 = 169, en küçük 14 → C', () =>
    expect(answerOf('triangles-564')).toBe('C'));
  it('565 — 11 < x + y < 7 + 9 = 16, 12..15 = 4 değer → C', () =>
    expect(answerOf('triangles-565')).toBe('C'));
  it('566 — a > 14 ve a < 23, 15..22 = 8 değer → C', () =>
    expect(answerOf('triangles-566')).toBe('C'));

  it('567 — I: 4 + 5 = 9, II: hₐ = 5 > c = 4, III–V üçgen belirtir → C', () =>
    expect(answerOf('triangles-567')).toBe('C'));
  it('568 — 60° te x² = 25 + 64 − 40 = 49, 3 < x < 7, 4 + 5 + 6 = 15 → B', () =>
    expect(answerOf('triangles-568')).toBe('B'));
  it('569 — |BC| = 15, 15 < toplam < 9 + 12 = 21, 16..20 = 5 değer → D', () =>
    expect(answerOf('triangles-569')).toBe('D'));
  it('570 — b > c > a, 8 < x + 3 < 14, 6 + 7 + 8 + 9 + 10 = 40 → A', () =>
    expect(answerOf('triangles-570')).toBe('A'));
  it('571 — m(BOC) > 90, x² > 74, x < 12, 9 + 10 + 11 = 30 → E', () =>
    expect(answerOf('triangles-571')).toBe('E'));
  ...
  it('591 — ABC: 3 < a < 17, BDC: 8 < a < 16, 15 − 9 = 6 → C', () =>
    expect(answerOf('triangles-591')).toBe('C'));
  it('592 — 6 < x + y < 5 + 8 = 13, 13 olamaz → E', () =>
    expect(answerOf('triangles-592')).toBe('E'));
  it('593 — m(ADB) > 90 ⇒ x² > 64 + 36 ⇒ x > 10, x < 14, 11 + 13 = 24 → C', () =>
    expect(answerOf('triangles-593')).toBe('C'));
  it('594 — ADC: 90 > 60 > 30 ⇒ |AD| > |DC|, |DC| > |AD| yanlış → C', () =>
    expect(answerOf('triangles-594')).toBe('C'));
  it('595 — |AC| < 14 ⇒ 13, x > 17 − 13 = 4, en küçük 5 → C', () =>
    expect(answerOf('triangles-595')).toBe('C'));
  it('596 — 2 < k < 14, 7k + 14 ≤ 63 ⇒ k ≤ 7, 6 < 3k ≤ 21 → B', () =>
    expect(answerOf('triangles-596')).toBe('B'));

  it('597 — |AB| > |AC| ⇒ m(ADC) < 90, 3x − 12 < 90 ⇒ x < 34, en büyük 33 → C', () =>
    expect(answerOf('triangles-597')).toBe('C'));
  it('598 — |BC|² < 64 + 225 = 289 ⇒ 7 < |BC| < 17, 8..16 = 9 → C', () =>
    expect(answerOf('triangles-598')).toBe('C'));
  it('599 — h = 12·16/20 = 9,6 ≤ |AD| < 16, 10..15 = 6 → C', () =>
    expect(answerOf('triangles-599')).toBe('C'));
  it('600 — |DE| = 4, m(BED) < 90 ⇒ x² > 49 − 16 = 33, 3 < x < 11, 6..10 = 5 → C', () =>
    expect(answerOf('triangles-600')).toBe('C'));
  it('601 — m(B) < 60 ⇒ (c − 15)(c + 7) > 0, 15 < c < 21, 18 → D', () =>
    expect(answerOf('triangles-601')).toBe('D'));
